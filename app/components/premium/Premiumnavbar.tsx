...
interface Props {
  // Pass the invitation slug so RSVP/FAQ link to the right pages
  slug?: string;
  // Optional override for logo (uses DB logoUrl or defaults to static files)
  logoUrl?: string;
}
...
export function PremiumNavbar({
  ...
  const NAV_RIGHT = [
    ...
    { label: "RSVP",    href: `/invitation/${slug}/rsvp`,     isPage: true  },
    { label: "FAQ",     href: `/invitation/${slug}/faq`,      isPage: true  },
  ];
  ...
  return (
    <>
      <header
        ...
        style={{
          background:   scrolled ? "rgba(247,243,238,0.96)" : "rgba(255,255,255,0.05)",
          backdropFilter: "blur(12px)",
          borderBottom: scrolled ? "1px solid rgba(201,169,126,0.22)" : "1px solid rgba(255,255,255,0.12)",
      ...
      >
        <nav className="mx-auto flex max-w-[1280px] items-center justify-between px-6 py-3">
          ...
          {/* Center logo — light version (hero) / dark version (scrolled) */}
          <a href="#home" className="shrink-0" onClick={(e) => { e.preventDefault(); scrollToHash("#home"); }}>
            <div className="relative w-[80px] h-[78px]">
              <Image
                src={logoUrl ?? "/images/premium/bw-logo.png"}
                alt="Couple monogram"
                fill sizes="80px"
          ...
          >
            <div className="relative w-[22px] h-[16px]">
              <span className={`absolute left-0 top-0 h-[2px] w-full bg-current transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${menuOpen ? "translate-y-[7px] rotate-45" : "translate-y-0 rotate-0"}`} />
              <span className={`absolute left-0 top-[7px] h-[2px] w-[16px] bg-current transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${menuOpen ? "opacity-0 translate-x-2" : "opacity-100 translate-x-0"}`} />
              <span className={`absolute left-0 bottom-0 h-[2px] w-full bg-current transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${menuOpen ? "-translate-y-[7px] -rotate-45" : "translate-y-0 rotate-0"}`} />
            </div>
          </button>
        </nav>
      </header>
      ...
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            ...
            >
              <Image src={logoUrl ?? "/images/premium/bw-logo.png"} alt="Couple monogram"
                width={70} height={68} className="object-contain opacity-60 mb-1" />
              ...
              {allNavItems.map(({ label, href, isPage }) =>
                isPage ? (
                  ...
                  <a key={label} href={href}
                    onClick={(e) => { e.preventDefault(); setMenuOpen(false); scrollToHash(href); }}
                    className="text-[28px] text-[#f7f3ee] hover:text-[#c9a97e] transition-colors duration-500 tracking-[0.02em]"
                    style={{ fontFamily: "var(--font-kapakana,'Cormorant Garamond',serif)", fontStyle: "italic" }}>
                    {label}
                  </a>
