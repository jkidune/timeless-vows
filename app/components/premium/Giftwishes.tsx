...
// ── Set this to the actual invitation slug ────────────────────────────
const INVITATION_SLUG = "barke-william-2026";
// ─────────────────────────────────────────────────────────────────────

const GIFT_ACCOUNTS = {
  mobile: { label: "SELCOM", name: "Tayamika Mattao", number: "+255 762 744 714", note: "Send to number · use your name as reference" },
  bank:   { label: "Bank Transfer", name: "Tayamika Mattao", bank: "CRDB Bank Tanzania", account: "0152349021800", branch: "Dar es Salaam Branch" },
};
...
export function GiftWishes() {
  ...
  useEffect(() => { getPublicWishes(INVITATION_SLUG).then(setWishes); }, []);

  const handleSubmit = async () => {
    if (!form.name.trim() || !form.message.trim()) { setError("Please fill in your name and message."); return; }
    setError(""); setLoading(true);
    const result = await submitGiftWish({ invitationSlug: INVITATION_SLUG, guestName: form.name, fromFamily: form.from || undefined, wishType: form.type || undefined, message: form.message });
      ...
      <section id="gift" className="bg-white px-6 py-[100px]">
        <div className="mx-auto max-w-[1280px]">
          <div className="flex flex-col items-center text-center mb-14 gap-4">
            ...
            <p className="max-w-[520px] text-[17px] italic text-[#b9927a]" style={{ fontFamily: "'Cormorant Garamond',serif" }}>Your presence is the greatest gift. But if your heart has words to share, Barke &amp; William would treasure them always.</p>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 border border-[#c9a97e]/20">
            ...
            <div className="bg-[#f7f3ee] p-10 lg:p-12 flex flex-col gap-10 border-r border-[#c9a97e]/15">
              <div>
                <div className="flex items-center gap-3 mb-6"><div className="h-px w-5 bg-[#c9a97e]/50" /><p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-[#a8795b]">Gift Transfer Details</p></div>
                <p className="text-[15px] italic text-[#b9927a] leading-[1.7] mb-6" style={{ fontFamily: "'Cormorant Garamond',serif" }}>If you wish to honour the couple with a monetary gift, please use any of the options below.</p>
                <div className="bg-white border border-[#c9a97e]/25 p-5 mb-3">
                  <div className="flex items-start justify-between mb-3"><p className="text-[10px] uppercase tracking-[0.22em] text-[#9d7760] font-medium">{GIFT_ACCOUNTS.mobile.label}</p><span className="text-[10px] uppercase tracking-[0.1em] px-2 py-1 bg-[#a8795b]/8 text-[#a8795b]">Mobile Money</span></div>
                  <div className="flex items-center justify-between gap-3"><p className="text-[clamp(18px,2.5vw,24px)] font-medium text-[#482612]" style={{ fontFamily: "var(--font-kapakana,'Cormorant Garamond',serif)" }}>{GIFT_ACCOUNTS.mobile.number}</p><CopyButton text={GIFT_ACCOUNTS.mobile.number} /></div>
                  <p className="text-[12px] text-[#b9927a] mt-1.5">Name: <span className="text-[#482612] font-medium">{GIFT_ACCOUNTS.mobile.name}</span></p>
                ...
                <div className="bg-white border border-[#c9a97e]/25 p-5">
                  <div className="flex items-start justify-between mb-3"><p className="text-[10px] uppercase tracking-[0.22em] text-[#9d7760] font-medium">{GIFT_ACCOUNTS.bank.label}</p><span className="text-[10px] uppercase tracking-[0.1em] px-2 py-1 bg-[#a8795b]/8 text-[#a8795b]">Bank</span></div>
                  <p className="text-[15px] font-semibold text-[#482612] mb-2">{GIFT_ACCOUNTS.bank.bank}</p>
                  <div className="flex items-center justify-between gap-3 mb-1.5"><div><p className="text-[11px] text-[#9d7760] uppercase tracking-[0.1em]">Account No.</p><p className="text-[16px] font-medium text-[#482612] tracking-[0.05em]">{GIFT_ACCOUNTS.bank.account}</p></div><CopyButton text={GIFT_ACCOUNTS.bank.account} /></div>
                  <p className="text-[12px] text-[#b9927a]">Name: <span className="text-[#482612] font-medium">{GIFT_ACCOUNTS.bank.name}</span></p>
                  <p className="text-[11px] italic text-[#c9a97e]/50 mt-1">{GIFT_ACCOUNTS.bank.branch}</p>
                </div>
              </div>
              <div>
                <div className="flex items-center gap-3 mb-5"><div className="h-px w-5 bg-[#c9a97e]/50" /><p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-[#a8795b]">Messages from loved ones</p></div>
                {wishes.length === 0
            ...
            <div className="relative bg-[#1c1713] p-10 lg:p-12">
              ...
              {phase === "form" ? (
                ...
                <div className="flex flex-col items-center text-center py-10 gap-6">
                  ...
                  <p className="text-[17px] italic text-[#e7d3bc]/65 leading-[1.7]" style={{ fontFamily: "'Cormorant Garamond',serif" }}>Your wish has been received and will be treasured by Barke &amp; William forever. 🤍</p>
