...
// Pure client component — no server imports
...
export function GalleryManager({ invitationId, images: initial }: Props) {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      ...
      if (file.size > 5 * 1024 * 1024) {
        setError(`${file.name} is over 5MB. Please compress it first.`);
        continue;
      }
      ...
      // Save to DB via API route (server-side, auth-checked)
      const res = await fetch("/api/gallery", {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ invitationId, url: publicUrl, sortOrder: images.length }),
      });
      ...
      if (res.ok) {
        ...
        setError("Failed to save image record. Please try again.");
      }
    }
  ...
  const handleDelete = async (image: GalleryImage) => {
    ...
    // Delete from DB via API route (server-side, ownership-checked)
    ...
    <div>
      ...
      >
        ...
        {uploading ? (
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-[14px] font-medium text-[#482612]">Click to upload images</p>
            <p className="text-[11px] text-[#b9927a]">JPG, PNG, WebP · Max 5MB each · Multiple allowed</p>
          </div>
        )}
      </div>

      {error && (
        <p className="mb-4 text-[12px] text-red-500/80 italic border border-red-100 bg-red-50 px-4 py-2">
          {error}
        </p>
      )}
      ...
      {images.length === 0 ? (
        <div className="bg-white border border-[#c9a97e]/18 p-12 text-center">
          ...
          <p className="text-[11px] text-[#c9a97e]/50 mt-1">Upload your first photos above</p>
        </div>
      ) : (
